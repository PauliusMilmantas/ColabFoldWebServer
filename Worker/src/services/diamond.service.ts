...
export class DiamondService implements Engine {
    ...
    async query(sequence: string, dbType: string): Promise<string> {
        ...
        const { stdout, err } = await this.exec('/app/engines/diamond/diamond blastp -d /app/databases/' + dbType + '/' + dbType + '.fasta -q input.fasta --sensitive --outfmt 0', { maxBuffer: 1024 * 1024 * 1024 });
